...
public class App {
  ...
  public static void main(String[] args) {
    ...
    //단지 유지보수를 좋게 하기위해 ArrayList와 LinkedList의 공통 분모를 뽑아서
    //만든 클래스가 List이다.
    // List클래스는 실제 작업을 하는 클래스가 아니다.
    //그럼에도 불구하고 개발자가 다음과같이 List객체를 사용하려 한다면 막을 수 없다.
    //=>BoardHandler의 경우 아무런 작업을 수행 하지 않을것이다.
    //=> 왜?List클래스에 정의 된 메서드는 아무것도 하지 않는다.
    //
    //해결책?
    //=>이렇게 generalization을 통해 만든 클래스의 경우
    //서브클래스에게 공통 분모를 물려주기 위한 용도로 사용된다.
    //=>이런류의 클래스는 직접 인스튼스를 생성하지 못하도록 해서
    //-> 직접 사용하는 것을 막아야한다.
    //이런 용도로 사용하는 문법이 "추상클래스(Abstract class)이다.
    
    //List클래스(AbstractList로 이름 변경함)를 추상 클래스로 만들면 , 다음과같이 인스턴스를 생성할 수 없다.
    //아예 다른 인스턴스 생성을 원천적으로 차단하는 효과가 있다.
