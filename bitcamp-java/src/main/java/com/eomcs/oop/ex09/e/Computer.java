...
public interface Computer {
    ...
    // 규칙을 추가하게 된다.
    //
//    void touch(); // vvvv추가하는 순간 기존 클래스들에서 컴파일 오류 발생!
    //
    // 문제점
    ...
    //  메서드 정의를 강제하는 것이다.
    //=>그러나 default메서드는 이미 구현되어있기 때문에
    //  클래스에서 반드시 정의 할 필요는 없다.
    //  즉 강제로 정의하게 만들 수 없는것이 문제이다.
    //=>그래서 default 메서드의 용도는
    //  기존에 정의 된 인터페이스에 새 규칙을 추가할 때
    //  기존 프로젝트에 영향을 끼치지 않기 위해 사용하는 것이다.
    //  따라서 새 인터페이스를 정의 할 때는 default의 사용을 자제해야한다.
}
