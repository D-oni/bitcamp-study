...
```
//이런 문제가 발생하는 이유?
- 다른 테이블의 데이터를 참조하는 경우 , 참조 데이터의 존재 유무를 검사하지 않기 때문
- 테이블의 데이터를 삭제할 때 다른 테이블이 참조하는 지 여부를 검사하지 않기 때문이다.

//해결책?
- 다른 테이블의 데이터를 참조하는 경우 해당 데이터의 존재 유무를 검사하도록 강제한다.
- 다른 데이터에 의해 참조되는 지 여부를 검사하도록 강제한다.
