...
public class ContextLoaderListener implements ApplicationContextListener {
  ...
  public void contextInitialized(Map<String, Object> context) {
    ...
    try {
      ...
      System.out.println("-----------------------------------");
      //@Component애노테이션이 붙은 객체를 찾는다.
      RequestMappingHandlerMapping handlerMapper =
          new RequestMappingHandlerMapping();
      String[] beanNames = appCtx.getBeansNamesForAnnotation(Component.class);
...
//        @RequestHandler가 붙은 메서드를 찾는다.
       ...
       if(method != null) {
        ...
        RequestHandler requestHandler= new RequestHandler(method,beanName);
  ...
  private Method getRequestHandler(Class<?> type) {
    //클라이언트 명령을 처리 할 메소드는 public 이기 때문에
    //class에서 public 메서드만 조사한다.
